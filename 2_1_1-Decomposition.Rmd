...
Now let's try STL with different options: First the standard polot


```{r stlstandard, warning=FALSE, fig.width=8.5, fig.height=3.4}
us_retail_employment |>
  model(STL(Employed)) |>
  components() |>
...
We see that the seasonal patterns change a bit over time. Lets say we do not want this, then we type:

```{r stlwindowperiodic, warning=FALSE, fig.width=8.5, fig.height=3.4}
us_retail_employment |>
...
Now we have the same pattern every year. We can also change the number of windows that are used. The smaller this number, the more the seasonal patterns is allowed to change (we essentially use smaller windows to estimate the seasonal component):

```{r stlwindow94, warning=FALSE, fig.width=8.5, fig.height=3.4}
us_retail_employment |>
...
We can also influence how the trend is plotted. Again, the window size allows us to influence how many windows are used to estimate the trend: The lower this number, the more ``wiggly'' the trend is estimated. In the extreme of a very high value, the trend becomes a straight line.

```{r stlwindow92, warning=FALSE, fig.width=8.5, fig.height=3.4}
us_retail_employment |>
  model(STL(Employed ~ season(window = 9) + trend(window=50))) |>
  components() |>
  autoplot() + labs(title = "STL decomposition: US retail employment")
```

We can also add the \textbf{robust} option:
```{r stlwindow93, warning=FALSE, fig.width=8.5, fig.height=3.4}
us_retail_employment |>
  model(STL(Employed ~ season(window = 15) + trend(window=15), robust=T)) |>
  components() |>
  autoplot() + labs(title = "STL decomposition: US retail employment")
```
...
In this case ``unusual'' observations get downweighted, so potential outliers will not influence the result too much.
...
\begin{itemize}
\item `trend(window = ?)` controls wiggliness of trend component.
\item `season(window = ?)` controls variation on seasonal component.
\item `season(window = 'periodic')` is equivalent to an infinite window (i.e. no change over time).
\end{itemize}

The defaults for STL are already pretty good in most cases:
