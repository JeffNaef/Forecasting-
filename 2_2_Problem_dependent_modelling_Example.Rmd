...
After differencing, the acf values are as we would expect for an independent series ($5\%$ of violations are expected). However since the acf is only able to deal with linear dependence, we also employ a more general, the distance autocorrelation measure to double check:
...
-   Now the series looks just like a white noise series:
    ...
    -   No distance autocorrelation outside the 95% limits.
