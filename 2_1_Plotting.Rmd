...
We now consider several ($n$) time series, in the form of different airports + classes:
...
We now consider only the $n=10$ time series of the economy class.
...
Instead we can also filter for the routes MEL-SYD $(n=3)$:
...
\textbf{Trend}: pattern exists when there is a long-term increase or decrease in the data.
  
  
\textbf{Cyclic}: pattern exists when data exhibit rises and falls that are \emph{not of fixed period} (duration usually of at least 2 years, e.g. business cycle).

\textbf{Seasonal}: pattern exists when a series is influenced by seasonal factors (e.g., the quarter of the year, the month, or day of the week). Big Example: Spikes of sales during Christmas.
...
Consider the following example of Australian electricity production:
...
```
Clearly there is a long term increase (i.e. trend). There is a clear seasonal component with regular peaks every 4 observations. Note that it would not have been necessary to check the plot for this, these features are pretty obvious for electricity data (expert knowledge).
...
Another Example concerns Retail Employment:
...
Similar patterns occur in stock prices:
...
```
However financial data is generally somewhat special, we will treat this in later contexts (e.g. random walks).

Another, more regular example is the annual number of Lynx trapped in Canada:
...
We now consider examples of how to analyse seasonality with plots,
...
We can also consider the subseries plots:
...
\item $\rho(t,t-1)=\text{Correlation}(Y_{t}, Y_{t-1})$
\item $\rho(t,t-2)=\text{Correlation}(Y_{t}, Y_{t-2})$
\item $\rho(t,t-3)=\text{Correlation}(Y_{t}, Y_{t-3})$ \\ \hspace*{0.5cm} $\vdots$
\end{itemize}


## Autocorrelation

Under weak stationarity the autocovariance, $\gamma(s,t) = \text{Cov}(Y_s, Y_t)$ only depence on the difference $h=|s-t|$. As such we shorten this to $\gamma(h)$ and similarly we denote the autocorrelation function as $\rho(h)$. They are then estimated as:

\begin{align*}
\hat{\gamma}(h) &= \frac{1}{T}\sum_{t=h+1}^T (Y_t-\bar{Y})(Y_{t-h}-\bar{Y}) \\[0.cm]
\text{and}\qquad
...
\end{align*}
  ...
  * $\hat{\rho}(1)$ indicates how successive values of $Y_t$ relate to each other
  * $\hat{\rho}(2)$ indicates how $Y_t$ values two periods apart relate to each other
  * $\hat{\rho}(k)$ is \textit{almost} the same as the sample correlation between $Y_t$ and $Y_{t-k}$.
...
```
  ...
  * $\hat{\rho}(4)$  higher than for the other lags due to **the seasonal pattern in the data**: peaks tend to be **4 quarters** apart and troughs tend to be **4 quarters** apart.
  * $\hat{\rho}(2)$ is more negative than for the other lags because troughs tend to be 2 quarters behind peaks.
...
Sampling distribution of $\hat{\rho}(k)$ for white noise data is asymptotically N(0,$1/T$).\pause

  *  95% of all $\hat{\rho}(k)$ for white noise must lie within $\pm 1.96/\sqrt{T}$.
