...
- $\epsilon_t \sim N(0, \Sigma_\epsilon)$ are idiosyncratic errors
- $u_t \sim N(0, \Sigma_u)$ are factor innovations
...
## Generate Factors with VAR(1) Dynamics
```{r generate-factors}
...
We now generate Factor Loadings $\Lambda$ and Observed Data
```{r generate-data}
...
## Visualize Sample of Observed Series
...
We can also look at the correlation between $Y_t$:
```{r}
cor(data_matrix)
```
...
## Define Prediction Interval Function

We generate prediction intervals by simulating from the estimated DFM:
...
### Coverage by Horizon
```{r coverage-by-horizon, fig.height=5}
...
```
We now visualize some of the forecasts with prediction intervals
