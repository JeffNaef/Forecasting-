...
est.pnbd.static.K1 <- latentAttrition(
  formula =~ holiday|holiday+sin_1+cos_1 ,
  family = pnbd,
  data = clv.dynamic,
  optimx.args = list(method = "Nelder-Mead"))

est.pnbd.static.K2 <- latentAttrition(
  formula =~ holiday|holiday+sin_1+cos_1 + sin_2 + cos_2 ,
  family = pnbd,
  data = clv.dynamic,
  optimx.args = list(method = "Nelder-Mead"))

est.pnbd.static.K3 <- latentAttrition(
  formula =~ Gender|holiday|holiday+sin_1+cos_1 + sin_2 + cos_2 + sin_3 + cos_3 ,
  family = pnbd,
  data = clv.dynamic,
  optimx.args = list(method = "Nelder-Mead"))
